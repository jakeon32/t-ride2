...
const WellihilliPark: React.FC = () => {
    ...
    const otherResorts = popularResorts.filter(resort => !resort.name.KR.includes("웰리힐리"));

    const heroSlides = [
        {
            id: 0,
            image: wellihilliImg,
            title: lang === 'KR' ? "수도권에서 가까운 강원권 스키장" : "Gangwon Ski Resort Near Seoul",
            desc: lang === 'KR'
                ? <>서울·수도권에서 약 1시간 40분 거리.<br />당일치기부터 1박 여행까지 부담 없이 즐기세요.</>
                : <>About 1 hour 40 minutes from Seoul/Metropolitan area.<br />Enjoy freely from day trips to overnight stays.</>
        },
        {
            id: 1,
            image: wellihilliImg,
            title: lang === 'KR' ? "다양한 파크 코스" : "Various Park Courses",
            desc: lang === 'KR'
                ? <>보더와 프리스타일 라이더를 위한<br />다양한 파크 아이템과 점프대를 갖추고 있습니다.</>
                : <>Equipped with various park items and jump ramps<br />for boarders and freestyle riders.</>
        },
        {
            id: 2,
            image: wellihilliImg,
            title: lang === 'KR' ? "콘도형 숙박 시설" : "Condo-style Accommodation",
            desc: lang === 'KR'
                ? <>스키장 바로 옆 콘도에서 편안한 휴식.<br />가족, 친구와 함께하는 완벽한 겨울 휴가.</>
                : <>Comfortable rest at the condo right next to the ski resort.<br />Perfect winter vacation with family and friends.</>
        }
    ];
    ...
    return (
        <div className="min-h-screen flex flex-col bg-slate-50">
            ...
            <div className="hidden md:block absolute top-24 left-0 w-full z-20 pointer-events-none">
                <Container>
                    <nav className="flex items-center text-sm text-white/80 font-medium pointer-events-auto drop-shadow-md">
                        ...
                        <span className="mx-2 opacity-60">&gt;</span>
                        <span className="text-white">{lang === 'KR' ? '웰리힐리파크' : 'Wellihilli Park'}</span>
                    </nav>
                </Container>
            </div>
            ...
            <StaticDetailHero
                ...
                mainTitle={lang === 'KR' ? "웰리힐리파크 스키장" : "Wellihilli Park Ski Resort"}
                description={lang === 'KR'
                    ? <>서울·수도권에서 비교적 가까운 강원권 스키장으로,<br className="hidden md:block" /> 다양한 파크 코스와 콘도형 숙박 시설을 갖춘 리조트입니다.</>
                    : <>A ski resort in Gangwon province relatively close to Seoul/Metropolitan area,<br className="hidden md:block" /> featuring various park courses and condo-style accommodation facilities.</>}
            />
                    ...
                    <Container>
                        <div className="mb-20 text-center md:text-left">
                            ...
                            <h2 className="text-h2 text-gray-900 mb-6 tracking-tight">
                                {lang === 'KR' ? '웰리힐리파크 이동 서비스' : 'Wellihilli Park Transfer Services'}
                            </h2>
                            <p className="text-slate-500 text-lg md:text-xl max-w-2xl leading-relaxed">
                                {lang === 'KR'
                                    ? <>서울·인천에서 웰리힐리파크까지. <br className="hidden md:block" /> 가장 쾌적하고 편안한 이동 방법을 선택하세요.</>
                                    : <>From Seoul/Incheon to Wellihilli Park. <br className="hidden md:block" /> Choose the most comfortable and convenient way to travel.</>}
                            </p>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                            <div>
                                ...
                                <p className="text-slate-600 font-light">
                                    {lang === 'KR'
                                        ? <>강원도 횡성군 둔내면 고원로 451<br />웰리힐리파크</>
                                        : <>451 Gowon-ro, Dunnae-myeon, Hoengseong-gun, Gangwon-do<br />Wellihilli Park</>}
                                </p>
                            </div>
                            <div>
