...
const VivaldiPark: React.FC = () => {
    ...
    const otherResorts = popularResorts.filter(resort => !resort.name.KR.includes("비발디"));

    const heroSlides = [
        {
            id: 0,
            image: leisureHeroImg,
            title: lang === 'KR' ? "수도권에서 가장 가까운 스키장" : "Nearest Ski Resort from Seoul",
            desc: lang === 'KR'
                ? <>서울에서 약 1시간 20분 거리.<br />부담 없는 당일치기 스키 여행의 최적지.</>
                : <>About 1 hour 20 minutes from Seoul.<br />The best destination for a burden-free day ski trip.</>
        },
        {
            id: 1,
            image: leisureHeroImg,
            title: lang === 'KR' ? "야간 스키의 명소" : "Famous for Night Skiing",
            desc: lang === 'KR'
                ? <>화려한 조명 아래 펼쳐지는 나이트 스키.<br />낮과는 다른 특별한 설원의 밤을 경험하세요.</>
                : <>Night skiing unfolding under colorful lights.<br />Experience a special snowy night different from the day.</>
        },
        {
            id: 2,
            image: leisureHeroImg,
            title: lang === 'KR' ? "가족·커플 인기 리조트" : "Popular Resort for Families & Couples",
            desc: lang === 'KR'
                ? <>스키장, 워터파크, 다양한 숙박 시설까지.<br />모두가 만족하는 올인원 겨울 휴양지.</>
                : <>Ski resort, water park, and various accommodation facilities.<br />An all-in-one winter vacation spot that satisfies everyone.</>
        }
    ];
    ...
    return (
        <div className="min-h-screen flex flex-col bg-slate-50">
            ...
            <div className="hidden md:block absolute top-24 left-0 w-full z-20 pointer-events-none">
                <Container>
                    <nav className="flex items-center text-sm text-white/80 font-medium pointer-events-auto drop-shadow-md">
                        ...
                        <span className="mx-2 opacity-60">&gt;</span>
                        <span className="text-white">{lang === 'KR' ? '비발디파크' : 'Vivaldi Park'}</span>
                    </nav>
                </Container>
            </div>
            ...
            <StaticDetailHero
                ...
                mainTitle={lang === 'KR' ? "비발디파크 스키월드" : "Vivaldi Park Ski World"}
                description={lang === 'KR'
                    ? <>수도권에서 가장 가까운 규모급 스키장 중 하나로,<br className="hidden md:block" /> 야간 운영과 다양한 숙박·워터파크 시설로 유명한 가족·커플 인기 리조트입니다.</>
                    : <>One of the large-scale ski resorts closest to the metropolitan area,<br className="hidden md:block" /> famous for night operations and various accommodation/water park facilities, popular among families and couples.</>}
            />
                    ...
                    <Container>
                        <div className="mb-20 text-center md:text-left">
                            ...
                            <h2 className="text-h2 text-gray-900 mb-6 tracking-tight">
                                {lang === 'KR' ? '비발디파크 이동 서비스' : 'Vivaldi Park Transfer Services'}
                            </h2>
                            <p className="text-slate-500 text-lg md:text-xl max-w-2xl leading-relaxed">
                                {lang === 'KR'
                                    ? <>서울·인천에서 비발디파크까지. <br className="hidden md:block" /> 가장 쾌적하고 편안한 이동 방법을 선택하세요.</>
                                    : <>From Seoul/Incheon to Vivaldi Park. <br className="hidden md:block" /> Choose the most comfortable and convenient way to travel.</>}
                            </p>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                            <div>
                                ...
                                <p className="text-slate-600 font-light">
                                    {lang === 'KR'
                                        ? <>강원도 홍천군 서면 한치골길 262<br />비발디파크</>
                                        : <>262 Hanchigol-gil, Seo-myeon, Hongcheon-gun, Gangwon-do<br />Vivaldi Park</>}
                                </p>
                            </div>
                            <div>
