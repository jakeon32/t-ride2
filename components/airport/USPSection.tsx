                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-8">
                    {uspItems.map((item, idx) => (
                        <div key={idx} className="group bg-[#34353d] md:bg-[#111] p-6 md:p-10 border border-white/5 hover:border-[var(--color-accent)] transition-all duration-300 text-center md:text-left">
                            <div className="mb-6 md:mb-8 text-white/20 group-hover:text-[var(--color-accent)] transition-all transform group-hover:scale-110 duration-500 ease-out flex justify-center md:justify-start">
                                ...
                                <div className="w-10 md:w-12 h-10 md:h-12 [&>svg]:w-full [&>svg]:h-full">
                                    {item.icon}
                                </div>
                            </div>

                            {/* L3-B: Card Title Large */}
                            <h3 className="text-base md:text-xl font-display font-bold text-white mb-4 md:mb-6 tracking-wide uppercase">
                                {item.title}
                            </h3>

                            {/* L4-B: Body Regular */}
                            <div className="text-xs md:text-base text-slate-500 space-y-1 font-light leading-relaxed group-hover:text-slate-400 transition-colors">
                                <p>{item.line1}</p>
                                <p>{item.line2}</p>
                                {/* L6-B: Caption */}
